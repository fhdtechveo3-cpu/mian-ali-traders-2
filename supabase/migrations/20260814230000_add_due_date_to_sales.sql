-- Migration to add due_date to sales table and update create_sale RPC
ALTER TABLE public.sales ADD COLUMN IF NOT EXISTS due_date DATE DEFAULT NULL;
...
BEGIN
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    v_item_price := (it->>'price')::NUMERIC;

    SELECT COALESCE(purchase_price, 0) INTO v_static_cost
    FROM public.products
    WHERE id = v_pid;

    v_sub := v_sub + (v_req_qty * v_item_price);
    v_cost := v_cost + (v_req_qty * v_static_cost);
  END LOOP;

  INSERT INTO public.sales(
    invoice_number, branch_id, customer_id, customer_name, customer_phone, subtotal, discount, total,
    cost_total, profit, paid_amount, remaining_amount, payment_method, notes, created_by, due_date
  )
  VALUES (
    ...
    COALESCE(_payment_method,'Cash'), _notes, auth.uid(), _due_date
  )
  ...
  FOR it IN SELECT * FROM jsonb_array_elements(_items) LOOP
    ...
    VALUES (v_sale_id, v_pid, it->>'product_name', v_req_qty, v_item_price, v_static_cost, v_req_qty * v_item_price);
